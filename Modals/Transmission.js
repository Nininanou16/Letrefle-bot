...
module.exports = async (Client, interaction) => {
  let transmissionServer = await Client.guilds.fetch(
    Client.settings.transmissions.serverID
  );
  if (transmissionServer) {
    let transmissionChannel = await transmissionServer.channels.cache.get(
      Client.settings.transmissions.channelID
    );
    if (transmissionChannel) {
      // let username = interaction.user.username;
      let guildMember = await interaction.guild.members.fetch(
        interaction.user.id
      );
