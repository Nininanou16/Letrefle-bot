...
>>>>>>> master
      ...
      let announcementChannel = await mainGuild.channels.fetch(
...
>>>>>>> master
  ...
  updateAvailable: async (Client) => {
    ...
    if (mainGuild) {
      let channel = await mainGuild.channels.fetch(
        Client.settings.available.channelID
      );
      if (channel) {
        let msg = await channel.messages.fetch(
          Client.settings.available.messageID
        );
        if (msg) {
          let text = "";
  ...
  updateChannelsMessage: async (Client) => {
    ...
    if (guild) {
      let channel = await guild.channels.fetch(
        Client.settings.channels.channelID
      );
      if (channel) {
        let message = await channel.messages.fetch(
          Client.settings.channels.messageID
        );
        if (message) {
...
>>>>>>> master
  ...
  error: async (Client, error) => {
    ...
    let mainGuild = await Client.guilds.fetch(Client.settings.mainGuildID);
    if (mainGuild) {
      let logsChannel = await mainGuild.channels.fetch(
